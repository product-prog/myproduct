...
// Stockage global des positions pour simuler la mémoire serveur
// En production, ceci serait dans une base de données
let globalPositions: UserPositions[] = [];
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      // POST /api/localisation - Enregistrer une position
      if (url.pathname === "/api/localisation" && method === "POST") {
        ...
        let userPositions = globalPositions.find(position => position.userId === userId);
      ...
      // GET /api/localisation - Récupérer toutes les positions
      if (url.pathname === "/api/localisation" && method === "GET") {
        return Response.json(
          globalPositions,
          { headers: corsHeaders(origin) }
        );
